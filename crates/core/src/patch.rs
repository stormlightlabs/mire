...
impl Default for PatchLimits {
    fn default() -> Self {
        Self {
            max_bytes: DEFAULT_MAX_PATCH_BYTES,
        }
    }
}
...
impl<'a> PatchInput<'a> {
    ...
    pub fn new(bytes: &'a [u8], limits: PatchLimits) -> Result<Self, PatchError> {
        if bytes.len() > limits.max_bytes {
            return Err(PatchError::InputTooLarge {
                actual: bytes.len(),
    ...
    pub fn as_utf8(self) -> Result<&'a str, PatchError> {
        std::str::from_utf8(self.bytes).map_err(|error| PatchError::Encoding {
            encoding: "UTF-8",
...
mod tests {
    ...
    fn input_at_limit_is_accepted_without_text_conversion() {
        let bytes = [0xff, b'\n'];
        let input = PatchInput::new(&bytes, PatchLimits { max_bytes: 2 })
            .expect("input exactly at the limit is accepted");
        assert_eq!(input.as_bytes(), bytes);
    }

    #[test]
    fn oversized_input_has_an_explicit_error() {
        let error = PatchInput::new(b"123", PatchLimits { max_bytes: 2 })
            .expect_err("input over the limit is rejected");
        assert_eq!(
            error,
    ...
    fn invalid_utf8_is_rejected_without_replacement() {
        let input = PatchInput::new(b"valid\n\xff", PatchLimits::default())
            .expect("input is within the byte limit");
        let error = input
            .as_utf8()
