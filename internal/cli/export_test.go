...
func TestExportCommandWritesExplicitFormatAndProtectsDestination(t *testing.T) {
	...
	reviewCommand := NewRootCommand(Config{Stdout: &reviewOutput, Stderr: &reviewDiagnostics, StateDir: stateDir, WorkingDir: repositoryPath})
	...
	command := NewRootCommand(Config{Stdout: &output, Stderr: &diagnostics, StateDir: stateDir, WorkingDir: repositoryPath})
	...
	if !strings.Contains(diagnostics.String(), "sensitive") || !strings.Contains(output.String(), "Exported json review") {
