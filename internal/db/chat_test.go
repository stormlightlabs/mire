...
func TestContextBoundChatPersistsAcrossRestartAndRejectsUnscopedTurns(t *testing.T) {
	...
	identity := RepositoryIdentity{
		CanonicalIdentity: "/workspaces/chat",
		DisplayName:       "chat",
		DiscoveredGitDir:  "/workspaces/chat/.git",
	}
	...
	anchor := review.Anchor{
		...
		Layer:      snapshot.TreeSideTarget,
		Path:       "README.md",
		BlobDigest: "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
		HunkID:     "README.md#1",
		HunkDigest: "hunk-1",
	}
	if err := store.RegisterDiffAnchors(
		context.Background(),
		round.ID,
		frozen.ID,
	...
	model := &dbChatModel{
		response: review.ChatResponse{
			SchemaVersion: review.ChatResponseSchemaVersion,
			Body:          "The change is bound to the captured README hunk.",
		},
	}
	if _, err := store.SendChatTurn(
		context.Background(),
		request,
		model,
		review.ChatOptions{
			ModelRunOptions: review.ModelRunOptions{Retry: review.RetryPolicy{MaxAttempts: 1}, Now: clock},
		},
	); err != nil {
	...
	if restartedTimeline.Runs[0].Binding.Context.Primary.Kind != review.ChatReferenceDiffAnchor ||
		restartedTimeline.Messages[1].ReplyTo != restartedTimeline.Messages[0].ID {
		t.Fatalf(
			"restarted binding/provenance = %#v / %#v",
			restartedTimeline.Runs[0].Binding,
			restartedTimeline.Messages[1],
		)
	}
}
...
func TestFindingRevisionChatReferenceIsResolvedToActiveRound(t *testing.T) {
	...
	session, round, frozen, err := store.CreateCapturedSession(
		context.Background(),
		RepositoryIdentity{
			CanonicalIdentity: "/workspaces/chat-finding",
			DisplayName:       "chat-finding",
			DiscoveredGitDir:  "/workspaces/chat-finding/.git",
		},
		"Chat finding",
		testCapture(t),
	)
	...
	change := review.ChangeModel{
		...
		SnapshotID:     frozen.ID,
		SnapshotDigest: frozen.ManifestDigest,
		Digest:         "change-digest",
		Files: []review.FileChange{
			{
				Status:       "modified",
				TargetPath:   "README.md",
				TargetDigest: "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
				Hunks:        []review.Hunk{{ID: "README.md#1", Digest: "hunk-1", Available: true}},
			},
		},
	}
	...
	if _, err := store.SendChatTurn(
		...
		&dbChatModel{
			response: review.ChatResponse{
				SchemaVersion: review.ChatResponseSchemaVersion,
				Body:          "The finding is now scoped.",
			},
		},
		review.ChatOptions{ModelRunOptions: review.ModelRunOptions{Retry: review.RetryPolicy{MaxAttempts: 1}}},
	); err != nil {
