-- Rebuild the immutable snapshot graph so working-tree captures can store
-- HEAD, index, and final-worktree layers while preserving prior snapshots.
ALTER TABLE activity RENAME TO activity_v5;
ALTER TABLE operations RENAME TO operations_v5;
ALTER TABLE rounds RENAME TO rounds_v5;
ALTER TABLE snapshot_entries RENAME TO snapshot_entries_v5;
ALTER TABLE snapshot_changes RENAME TO snapshot_changes_v5;
ALTER TABLE snapshots RENAME TO snapshots_v5;
...
CREATE TABLE snapshots (
    ...
    kind TEXT NOT NULL CHECK (kind IN ('two_dot', 'three_dot', 'worktree')),
    ...
    ignore_policy TEXT NOT NULL DEFAULT '',
...
CREATE TABLE snapshot_layers (
    ...
    layer TEXT NOT NULL CHECK (layer IN ('base', 'target', 'head', 'index', 'worktree')),
...
CREATE TABLE snapshot_entries (
    snapshot_id TEXT NOT NULL,
    tree_side TEXT NOT NULL CHECK (tree_side IN ('base', 'target', 'head', 'index', 'worktree')),
    path TEXT NOT NULL,
...
INSERT INTO snapshots (
    ...
    target_oid, merge_base_oid, index_oid, object_format, context_policy_hash,
    ignore_policy, base_manifest_digest, target_manifest_digest, manifest_digest,
    complete, created_at
)
...
SELECT id, 'base', effective_base_oid, base_manifest_digest FROM snapshots_v5
UNION ALL
SELECT id, 'target', target_oid, target_manifest_digest FROM snapshots_v5;
...
DROP TABLE activity_v5;
DROP TABLE operations_v5;
DROP TABLE rounds_v5;
DROP TABLE snapshot_changes_v5;
DROP TABLE snapshot_entries_v5;
DROP TABLE snapshots_v5;
