...
func TestBuildAndRenderAreDeterministicAndDoNotEmbedSnapshotContent(t *testing.T) {
	...
	baseEntries := []snapshot.Entry{
		{Path: "a.txt", Kind: snapshot.EntryKindFile, Mode: 0o644, Size: 7, ContentDigest: baseObject.Digest},
	}
	targetEntries := []snapshot.Entry{
		{Path: "a.txt", Kind: snapshot.EntryKindFile, Mode: 0o644, Size: 6, ContentDigest: targetObject.Digest},
	}
	...
	session, round, _, err := store.CreateCapturedSession(
		ctx,
		db.RepositoryIdentity{
			CanonicalIdentity: "/tmp/export-fixture",
			DisplayName:       "fixture",
			DiscoveredGitDir:  "/tmp/export-fixture/.git",
		},
		"fixture",
		capture,
	)
	...
	if digest, digestErr := snapshot.OverallManifestDigest(
		reconstructed,
	); digestErr != nil ||
		digest != persisted.ManifestDigest {
		t.Fatalf(
			"reconstructed=%#v digest=%s err=%v persisted=%#v manifest=%s layers=%#v",
			reconstructed,
			digest,
			digestErr,
			persisted,
	...
	if value.Change.Digest == "" || !strings.Contains(value.DiffPatch, "before") ||
		!strings.Contains(value.DiffPatch, "after") {
...
func TestBundleContainsOnlyNamedFiles(t *testing.T) {
	...
	value := Review{
		...
		Session:          SessionDescriptor{ID: "session", RepositoryIdentity: "repo"},
		Round:            RoundDescriptor{ID: "round", Status: string(db.RoundStatusComplete)},
		SnapshotManifest: SnapshotManifest{ID: "snapshot", ManifestDigest: "manifest"},
