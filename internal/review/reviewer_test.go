...
func TestRunReviewPassesRetainsDuplicatesAndSeparatesIncompleteOutcomes(t *testing.T) {
	...
	large := append(
		[]byte(`{"schema_version":"mire/v1/review-candidates","candidates":[]}`),
	...
	model := &reviewerFixtureModel{responses: []reviewerFixtureResponse{
		{
			output: mustJSON(
				t,
				CandidateEnvelope{SchemaVersion: ReviewCandidateSchemaVersion, Candidates: []Candidate{}},
			),
		},
		{output: validJSON},
	...
	result, err := RunReviewPasses(context.Background(), change, model, ReviewerOpts{
		ModelRunOptions: ModelRunOptions{
			Retry: RetryPolicy{MaxAttempts: 1, RepairAttempts: 0},
	...
	if len(result.Candidates) != 2 || result.Candidates[0].ID == result.Candidates[1].ID ||
		result.Candidates[0].Fingerprint != result.Candidates[1].Fingerprint {
	...
	if !hasDiagnostic(result.Diagnostics, DiagnosticMalformedCandidates) ||
		!hasDiagnostic(result.Diagnostics, DiagnosticUnsupportedProse) ||
		!hasDiagnostic(result.Diagnostics, DiagnosticOutputBudget) {
...
func TestRunReviewPassesBoundsRetrievalAndRetainsItsAuditTrail(t *testing.T) {
	...
	result, err := RunReviewPasses(context.Background(), change, model, ReviewerOpts{
		...
		}, Passes: []PlannedPass{{Name: "tests", Order: 0, Applicable: true, Reason: "fixture"}},
		Retriever: retriever, Budgets: map[string]PassBudget{"tests": {MaxArtifacts: 2}},
	})
	if err != nil {
		t.Fatalf("RunReviewPasses() error = %v", err)
	}
	if len(result.Coverage.RetrievedArtifacts) != 3 || result.Coverage.RetrievedArtifacts[1].Kind != "test" ||
		!result.Coverage.RetrievedArtifacts[2].Excluded {
		t.Fatalf(
			"retrieved artifacts = %#v, want changed code, included context, and excluded context",
			result.Coverage.RetrievedArtifacts,
		)
	}
	...
	if result.Coverage.RetrievedArtifacts[0].RunID == "" || result.Coverage.RetrievedArtifacts[1].RunID == "" ||
		result.Coverage.RetrievedArtifacts[1].Digest == "" {
...
func TestRunReviewPassesInvalidCandidateIsDiagnosticNotFinding(t *testing.T) {
	...
	result, err := RunReviewPasses(
		context.Background(),
		change,
		model,
		ReviewerOpts{
			ModelRunOptions: ModelRunOptions{Retry: RetryPolicy{MaxAttempts: 1}},
			Passes:          []PlannedPass{{Name: "tests", Order: 0, Applicable: true}},
		},
	)
	if err != nil {
		t.Fatalf("RunReviewPasses() error = %v", err)
