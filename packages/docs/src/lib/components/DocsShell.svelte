...
<Seo title={`${doc.title} · Mire docs`} description={doc.description} pathname={`/docs/${doc.slug}/`} />
...
<style>
	...
	.doc-content :global(pre) {
		...
		color: var(--code-ink);
		box-shadow: 0.35rem 0.35rem 0 var(--teal-bright);
	}
	...
	@media (max-width: 520px) {
		...
		.doc-content :global(pre) {
			...
			margin-left: -0.25rem;
			box-shadow: 0.2rem 0.2rem 0 var(--teal-bright);
		}
	}
</style>
